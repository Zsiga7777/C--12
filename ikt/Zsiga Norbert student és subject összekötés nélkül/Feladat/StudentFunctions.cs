...
public static class StudentFunctions
{
    ...
    {
        List<StudentEntity> students =await dbContext.Students.ToListAsync();
        ulong input = 0;

        do
        {         
            input = ExtendentConsole.ReadUlong("Kérem a oktatási azonosítóját(11 számjegy):  ", 9999999999, 100000000000);
            if (students.Any(x => x.EducationalID == input))
            {
                Console.WriteLine("Ilyen oktatási azonosító már létezik.");
                await Task.Delay(2000);
            }
        } while (students.Any(x => x.EducationalID == input));
